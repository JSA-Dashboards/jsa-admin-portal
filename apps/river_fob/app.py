...
var renamed to RIVERFOB_DATABASE_URL, and set_page_config removed in favor of
the shared shell's single call in Home.py.
"""
import base64
...
# Renamed on disk (fob_model.py -> riverfob_fob_model.py, etc.) for the JSA
# Home Page merge: basis_tracker has its OWN same-named modules, and Python's
# sys.modules cache would silently hand one page's import to the other page
# once either had loaded first in this shared process — a real cross-app data
# bug, not just an import error. See jsa-home-page's plan doc.
import riverfob_fob_model as M
import riverfob_seed_data as S
import db
import paste_parse
import fob_pdf
import fob_excel
...
try:
    ...
    # DATABASE_URL in the merged JSA Home Page process (see db.py:_database_url).
    if "RIVERFOB_DATABASE_URL" in st.secrets and not os.environ.get("RIVERFOB_DATABASE_URL"):
        os.environ["RIVERFOB_DATABASE_URL"] = st.secrets["RIVERFOB_DATABASE_URL"]
except Exception:
...
@st.cache_resource
def _ensure_db():
    # Allow RIVERFOB_DATABASE_URL via Streamlit secrets (falls back to local SQLite).
    try:
        if "RIVERFOB_DATABASE_URL" in st.secrets:
            os.environ["RIVERFOB_DATABASE_URL"] = st.secrets["RIVERFOB_DATABASE_URL"]
    except Exception:
        pass
    db.init_db()
...
# --- top controls -----------------------------------------------------------
# Moved out of the sidebar (JSA Admin Portal shell has no left sidebar for
# this page) into a horizontal bar at the top — same widgets, same session
# keys, same downstream variables (as_of / view_choice / HIST_DATE); only the
# layout changed.
_logo_sb = (f'<img src="{LOGO_URI}" style="height:22px;vertical-align:middle;margin-right:8px" '
            f'alt="JSA">' if LOGO_URI else "")
st.markdown(
    f'<div style="padding:2px 0 10px;border-bottom:2px solid #0693e3;margin-bottom:14px">'
    f'{_logo_sb}<span style="font-weight:700;color:#32373c;font-size:1rem">River FOB Portal</span>'
    f'<span style="color:#666;font-size:0.8rem;margin-left:10px">Commodity &amp; Ag Risk Management</span>'
    f'</div>',
    unsafe_allow_html=True,
)
...
if VIEW_ONLY:
    ...
    _c1, _c2, _c3, _c4, _c5, _c6 = st.columns([1.3, 1.1, 0.8, 0.8, 1.4, 0.9])
    with _c1:
        as_of = st.date_input(
    ...
    with _c2:
        st.session_state.interest_pct = st.number_input(
            "Interest rate (%)", value=float(st.session_state.interest_pct),
            step=0.25, format="%.2f",
            help="Annual rate used for % Full Carry; storage is per-commodity "
                 "on the Inputs tab.")
    _arch_dates_all = db.list_dates()
    _arch_years = sorted({d[:4] for d in _arch_dates_all}, reverse=True) if _arch_dates_all else []
    with _c3:
        _arch_fy = st.selectbox("Archive year", ["All"] + _arch_years, key="arch_year")
    _arch_pool = (_arch_dates_all if _arch_fy == "All"
                  else [d for d in _arch_dates_all if d[:4] == _arch_fy])
    _arch_lbl_to_num = {_MONTH_ABBR[int(m) - 1]: m for m in sorted({d[5:7] for d in _arch_pool})}
    with _c4:
        _arch_fm = st.selectbox("Month", ["All"] + list(_arch_lbl_to_num), key="arch_month")
    if _arch_fm != "All" and _arch_fm in _arch_lbl_to_num:
        _arch_pool = [d for d in _arch_pool if d[5:7] == _arch_lbl_to_num[_arch_fm]]
    with _c5:
        view_choice = st.selectbox(
            "View archived date", ["✏️ Working (live)"] + _arch_pool,
            index=1 if _arch_pool else 0,                 # default: latest saved day
            help="Filter by year/month, then pick a day (opens read-only). "
                 "Choose '✏️ Working (live)' to edit on the Inputs tab.")
    with _c6:
        st.markdown("<div style='height:28px'></div>", unsafe_allow_html=True)
        if st.button("↺ Reset to seed", help="Reset inputs to seed",
    ...
    st.caption(
        ...
        f"as-of month) · Archive: CIF + barge freight · {DB_BACKEND} · Enter "
        f"CIF & barge freight on the **📝 Inputs** tab — the commodity tabs "
        f"update live; *what-if* until you **Save**.")
...
# --- export (moved out of the sidebar into a top-level expander) -----------
if not VIEW_ONLY:                       # downloads hidden in read-only mode
    with st.expander("📤 Export FOB Sheet (PDF / Excel)", expanded=False):
        ...
        with _exp_c1:
            try:
                ...
                # "Save to folder" only works when the app runs on a machine that
                # can see the SharePoint-synced folder — hidden on the cloud
                # (Linux) deploy.
                if os.path.isdir(FOB_SAVE_DIR):
                    ...
                    st.caption("💡 Run the app locally to enable **Save to FOB "
                               "folder**; on the cloud, use the download button.")
        ...
        with _exp_c2:
            try:
                st.download_button(
                    ...
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    use_container_width=True,
                    help="One workbook, tab named by date: Corn, Soybeans, Wheat "
                         "stacked with a blank row between each.")
            except Exception as e:
