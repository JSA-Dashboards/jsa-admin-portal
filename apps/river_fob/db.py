...
def _database_url():
    # Renamed from DATABASE_URL to RIVERFOB_DATABASE_URL in this merged-app
    # copy — basis_tracker's database.py used the generic DATABASE_URL name
    # for a DIFFERENT Supabase project; sharing one process (JSA Home Page)
    # means the generic name must not collide.
    return os.environ.get("RIVERFOB_DATABASE_URL", "").strip()
